/**
 * SplashCursor – Full-screen WebGL fluid simulation that reacts to cursor movement.
...
const SplashCursor: FC<SplashCursorProps> = ({
  SIM_RESOLUTION = 128,
  DYE_RESOLUTION = 1440,
  ...
  useEffect(() => {
    ...
    /* Hoisted refs so cleanup can access them outside the try block */
    ...
    try { // Guard entire WebGL init — if anything fails, bail gracefully
    ...
    function initFramebuffers() {
      ...
      // Guard: if any required texture format is unsupported, bail
      if (!rgba || !rg || !r) {
        console.warn('[SplashCursor] Required WebGL texture formats not supported — skipping.');
        return;
      }
    ...
    function updateColors(dt: number) {
      ...
      if (colorUpdateTimer >= 1) {
        ...
        pointers.forEach((p) => { p.color = [generateColor().r, generateColor().g, generateColor().b]; });
      }
    }
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
      ...
      pointer.deltaX = 0;
      pointer.deltaY = 0;
      pointer.color = (() => { const c = generateColor(); return [c.r, c.g, c.b]; })();
    }
    ...
    updateFrame();
      ...
      console.error('[SplashCursor] WebGL init failed — disabling effect:', err);
      isActive = false;
    }
